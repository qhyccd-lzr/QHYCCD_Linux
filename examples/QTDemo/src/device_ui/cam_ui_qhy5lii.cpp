...
cam_ui_qhy5lii::cam_ui_qhy5lii( ccamera_base *pcam, QWidget *parent ) :
    ui_device_base(pcam, parent)
{
        ui.setupUi( this->ui_device_base::m_hw_ui );
        this->ui_device_base::m_hw_ui->setWindowTitle( tr("HW params") );

        m_params = static_cast<cam_qhy5lii_params *>(m_camera->alloc_params_object());
...
cam_ui_qhy5lii::~cam_ui_qhy5lii()
{
        delete m_params;
}


void cam_ui_qhy5lii::onHwOkClick()
{
        if( is_valid() )
...
bool cam_ui_qhy5lii::is_valid( void )
{
 int gain;
 double exposure;
 int binning;
...
bool cam_ui_qhy5lii::apply( void )
{
        if( !is_valid() )
                return false;
...
void cam_ui_qhy5lii::fill_interface( void )
{
