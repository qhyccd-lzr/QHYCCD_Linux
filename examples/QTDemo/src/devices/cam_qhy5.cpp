...
//-------------------------------------------- QHY5 ---------------------------------------------------------
// QHY5 stuff
ccamera_qhy5::ccamera_qhy5() : m_qhy5_obj( NULL )
{
...
camera_model::model ccamera_qhy5::get_model( void ) const
{
	return camera_model::qhy5;
}


int ccamera_qhy5::open_device( void )
{
	...
	if( ret == EXIT_SUCCESS )
	{
		...
		ret = m_qhy5_obj->set_params( p.exposure,
				p.binn,
				p.gain,
				1, // first_time init
				&p.out_frame_width,
		...
		if( ret == EXIT_SUCCESS )
		{
			m_low_params  = p;
			m_initialized = true;
			log_i( "Connected to QHY5 camera" );
		}
		else
			close_device();
	}
...
void ccamera_qhy5::close_device( void )
{
...
cam_base_params *ccamera_qhy5::alloc_params_object( void ) const
{
	return new cam_qhy5_params();
}


bool ccamera_qhy5::set_params( cam_base_params *params )
{
 cam_qhy5_params *h_params = NULL;
 qhy5_params_t p;
 bool ret = false;


	h_params = dynamic_cast<cam_qhy5_params *>(params);
	...
	// goto thread
	ret = set_thread_task( MK_CMD("@SETPAR"), (char*)&p, sizeof(qhy5_params_t), NULL, 0, true );
	if( !ret )
	{
		log_e("ccamera_qhy5::set_params: set_thread_task error");
		return false;
	}
...
bool ccamera_qhy5::get_params( cam_base_params *params )
{
 cam_qhy5_params *dst = dynamic_cast<cam_qhy5_params *>(params);
 bool ret = false;
	...
	// goto thread
	ret = set_thread_task( MK_CMD("@GETPAR"), NULL, 0, (char*)&m_low_params, sizeof(qhy5_params_t), true );
	if( !ret )
	{
		log_e("ccamera_qhy5::get_params: set_thread_task error");
		return false;
	}
...
bool ccamera_qhy5::get_params_copy( cam_base_params *params ) const
{
 cam_qhy5_params *dst = dynamic_cast<cam_qhy5_params *>(params);
...
const cam_base_params &ccamera_qhy5::get_params_ref( void ) const
{
 return m_high_params;
}


bool ccamera_qhy5::get_frame( void )
{
 ...
 bool ret = false;
	...
	if( !ret )
	{
		log_e("ccamera_qhy5::get_frame: set_thread_task error");
		return false;
	}
...
bool ccamera_qhy5::get_frame_params( frame_params_t *params )
{
 frame_params_t frame_params;
	 ...
	 // for QHY5 buffer is larger than wd*ht*pixsize
...
int ccamera_qhy5::do_command( cam_task_t *task )
{
	qhy5_params_t *p = NULL;
	...
	{
		if( task->param_len != sizeof(qhy5_params_t) || !(p = reinterpret_cast<qhy5_params_t *>(task->param)) )
		{
			log_e( "ccamera_qhy5::do_command(): CMD = %.*s has inconsistent params.", task->cmd_len, task->cmd );
			return 1;
		}

		int res = m_qhy5_obj->set_params( p->exposure, p->binn, p->gain, 0, &p->out_frame_width, &p->out_frame_height, &p->out_buffer_size );
		if( res != EXIT_SUCCESS )
		{
			log_e( "ccamera_qhy5::do_command(): CMD = %.*s failed.", task->cmd_len, task->cmd );
			return 2;
		}
	...
	{
		if( task->out_len != sizeof(qhy5_params_t) || !(p = reinterpret_cast<qhy5_params_t *>(task->out)) )
		{
			log_e( "ccamera_qhy5::do_command(): CMD = %.*s has inconsistent params.", task->cmd_len, task->cmd );
			return 3;
		}
		...
		memcpy( task->out, &lp, sizeof(qhy5_params_t) );
	...
	if( strncmp( task->cmd, "@IMG", task->cmd_len ) == 0 )
	{
		...
		int res = m_qhy5_obj->start_exposure( m_low_params.exposure );
		if( res != EXIT_SUCCESS )
		{
			log_e( "ccamera_qhy5::do_command(): CMD = %.*s failed. Unable to start exposure", task->cmd_len, task->cmd );
			return 5;
		}
		...
		while( (now = progress_timer.gettime()) < (unsigned long)m_high_params.Exposition )
		{
			...
			{
				//hardware BREAK
				int res = m_qhy5_obj->reset_camera(); //system( "utils/qhy5reset.sh" );
				if( res != EXIT_SUCCESS )
				{
				...
				{
					ret = m_qhy5_obj->set_params( m_low_params.exposure,
							m_low_params.binn,
							m_low_params.gain,
							0,
							NULL,
					...
					if( res != EXIT_SUCCESS )
					{
						log_e( "ccamera_qhy5::do_command(): set_params() failed after reset" );
					}
				}
				break_done = true;
				break;
		...
		if( !break_done )
		{
			...
			if( !raw_buffer )
			{
				log_e( "ccamera_qhy5::do_command(): CMD = %.*s failed. Unable to allocate raw data buffer", task->cmd_len, task->cmd );
				return 6;
			}
			int res = m_qhy5_obj->read_exposure( raw_buffer, m_low_params.out_buffer_size );
			if( res != EXIT_SUCCESS )
			{
				free( raw_buffer );
				log_e( "ccamera_qhy5::do_command(): CMD = %.*s failed.", task->cmd_len, task->cmd );
			...
			{
				for( unsigned int col = col_start; col < QHY5_WIDTH_B1 + col_start; col++ )
				{
					if( col-20 == row )
						src[row * QHY5_MATRIX_WIDTH + col] = 128+rand()%32;
				}
			}
#endif

			// copy
			unsigned int col_end = QHY5_WIDTH_B1 + col_start;
			...
			case 2:
				for( unsigned int row = 0; row < row_end; row+=2 )
				{
					...
					{
						unsigned int off = row * QHY5_MATRIX_WIDTH + col;
						unsigned int val = src[off] + src[off+1] + src[off+QHY5_MATRIX_WIDTH] + src[off+QHY5_MATRIX_WIDTH+1];
				...
				break;
			default:
				log_e( "ccamera_qhy5::do_command(): Unsupported binning = %d", m_high_params.Binning );
			}
