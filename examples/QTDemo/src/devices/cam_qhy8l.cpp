...
const int ccamera_qhy8l::m_binn_loading_time[QHY8L_SPEED_CNT][QHY8L_BINN_CNT] = {
											{9000, 4500, 2250, 2250 },
											{4500, 1125, 1200, 1200 },
											};
const int ccamera_qhy8l::m_binn_loading_size[QHY8L_BINN_CNT] = { 13513728, 3379712, 1690624, 1690624 };

ccamera_qhy8l::ccamera_qhy8l() :
	m_handle( NULL )
{
	int ret = lusb::initialize();

	if( ret != 0 )
		log_e( "ccamera_qhy8l::ccamera_qhy8l(): Could not initialize libusb" );
...
camera_model::model ccamera_qhy8l::get_model( void ) const
{
	return camera_model::qhy8l;
}


int ccamera_qhy8l::open_device( void )
{
	...
	if( lusb::is_initialized() )
		log_i( "ccamera_qhy8l::open_device(): Success" );
#else
	if( m_handle == NULL && lusb::is_initialized() )
	{
		if( (m_handle = locate_device(QHY8L_VENDOR_ID, QHY8L_PRODUCT_ID)) == NULL )
		{
			log_e( "ccamera_qhy8l::open_device(): Could not open the QHY8L device" );
			return EXIT_FAILURE;
		}
	}
#endif
	...
	do
	{
		...
		if( ret != EXIT_SUCCESS )
		{
			log_e( "ccamera_qhy8l::open_device(): set_params() failed" );
			break;
		}

		m_low_params  = p;
		m_initialized = true;
		log_i( "Connected to QHY8L camera" );
		return EXIT_SUCCESS;
	}while( 0 );
...
void ccamera_qhy8l::close_device( void )
{
	if( m_handle )
	{
...
cam_base_params *ccamera_qhy8l::alloc_params_object( void ) const
{
	return new cam_qhy8l_params();
}


bool ccamera_qhy8l::set_params( cam_base_params *params )
{
 ...
 bool ret = false;
	...
	if( !ret )
	{
		log_e("ccamera_qhy8l::set_params: set_thread_task error");
		return false;
	}
...
bool ccamera_qhy8l::get_params( cam_base_params *params )
{
...
bool ccamera_qhy8l::get_params_copy( cam_base_params *params ) const
{
 cam_qhy8l_params *dst = dynamic_cast<cam_qhy8l_params *>(params);
...
const cam_base_params &ccamera_qhy8l::get_params_ref( void ) const
{
 return m_high_params;
}


bool ccamera_qhy8l::exec_slow_ambiguous_synchronous_request( int req_num,
														...
														std::map< std::string, std::string > *result )
{
	...
	switch( req_num )
	{
	case ccamera_qhy8l::temp_voltage_req:
	{
		(void)params;
		...
		if( !ret )
		{
			log_e("ccamera_qhy8l::exec_slow_ambiguous_synchronous_request(): req_num = %d: set_thread_task error", req_num );
			return false;
		}
		if( result )
		{
		...
		log_i( "ccamera_qhy8l::exec_slow_ambiguous_synchronous_request(): req = @GETTEMP: temp = %.1f, volt = %d", info_out.temperature, info_out.voltage );
		break;
	}
	default:
		log_e( "ccamera_qhy8l::exec_slow_ambiguous_synchronous_request(): Unknown request nubler" );
		return false;
...
std::vector<float> ccamera_qhy8l::get_debayer_settings( void ) const
{
	...
	log_i( "debayer settings.size = %d", settings.size() );

	return settings;
}


bool ccamera_qhy8l::get_frame( void )
{
 ...
 bool ret = false;
	...
	if( !ret )
	{
		log_e("ccamera_qhy8l::get_frame: set_thread_task error");
		return false;
	}
...
bool ccamera_qhy8l::get_frame_params( frame_params_t *params )
{
 frame_params_t frame_params;
...
int ccamera_qhy8l::do_command( cam_task_t *task )
{
	...
	if( strncmp( task->cmd, "@SETPAR", task->cmd_len ) == 0 )
	{
		...
		{
			log_e( "ccamera_qhy8l::do_command(): CMD = %.*s has inconsistent params.", task->cmd_len, task->cmd );
			return 1;
		}
		...
		if( res != EXIT_SUCCESS )
		{
			log_e( "ccamera_qhy8l::do_command(): CMD = %.*s failed.", task->cmd_len, task->cmd );
			return 2;
		}
	}
	else
	...
	if( strncmp( task->cmd, "@IMG", task->cmd_len ) == 0 )
	{
		...
		if( res != EXIT_SUCCESS )
		{
			log_e( "ccamera_qhy8l::do_command(): CMD = %.*s failed. Unableto start exposure", task->cmd_len, task->cmd );
			return 5;
		}
				...
				for( int i = 0;i < ccamera_base::try_reset_cnt;i++ )
				{
					...
					else
					{
						...
						if( res == EXIT_SUCCESS )
							...
							log_e( "ccamera_qhy8l::do_command(): set_params() failed after reset. Trying #%d", i+1 );
					}
				}
				break_done = true;
				break;
		...
		if( !break_done )
		{
			...
			if( !raw_buffer )
			{
				log_e( "ccamera_qhy8l::do_command(): CMD = %.*s failed. Unableto allocate raw data buffer", task->cmd_len, task->cmd );
				return 6;
			}
			...
			int res = libusb_bulk_transfer( m_handle,
											0x82,
											raw_buffer,
											m_binn_loading_size[ binn_idx ],
											&result,
											m_binn_loading_time[ speed_idx ][ binn_idx ] );
			log_i( "downloading time. time = %d", progress_timer.gettime() - start_tm );
			if( res < 0 || result != m_binn_loading_size[ binn_idx ] )
			{
				if( abs(m_binn_loading_size[ binn_idx ] - result) >  m_low_params.out_frame_width*4 )
				{
					free( raw_buffer );
					log_e( "ccamera_qhy8l::do_command(): CMD = %.*s failed. libusb_bulk_transfer() failed: res = %d, expected = %d bytes, got = %d bytes",
					...
					log_e( "ccamera_qhy8l::do_command(): CMD = %.*s Warning!!! res = %d, expected = %d bytes, got = %d bytes",
							task->cmd_len, task->cmd,
							res,
							m_binn_loading_size[ binn_idx ],
							result );
...
libusb_device_handle *ccamera_qhy8l::locate_device( unsigned int vid, unsigned int pid )
{
...
int ccamera_qhy8l::ctrl_msg( int request_type, int request, unsigned int value, unsigned int index, unsigned char *data, int len )
{
	int result;
...
unsigned char ccamera_qhy8l::MSB(unsigned int i)
{
	unsigned int j;
	j = (i&~0x00ff)>>8;
	return j;
...
unsigned char ccamera_qhy8l::LSB( unsigned int i )
{
	unsigned int j;
	j = i&~0xff00;
	return j;
...
int ccamera_qhy8l::set_params( int exposuretime, int binn, int gain, int offset, int speed, int amp, int pwm, int *out_width, int *out_height, int *out_buffer_size, int *out_top_skip_pix )
{
	...
	default:
		log_e( "Setting parameters... Unsupported binning value = %d", binn );
		return 2;
	}
	...
	//log_i( "PatchNumber: %d", PatchNumber );
	...
	if( ret != EXIT_SUCCESS )
	{
		log_e( "ccamera_qhy8l::set_params(): ctrl_msg() failed" );
		return ret;
	}
	...
	if( ret != EXIT_SUCCESS )
	{
		log_e( "ccamera_qhy8l::set_params(): set_dc201() failed" );
		return ret;
	}
	...
	if( ret != EXIT_SUCCESS )
	{
		log_e( "ccamera_qhy8l::set_params(): get_info() failed" );
		return ret;
	}
...
int ccamera_qhy8l::get_info( struct info_s *info_out, bool dump )
{
	short dc = 0;
	int ret = EXIT_SUCCESS;
	...
	if( ret != EXIT_SUCCESS )
	{
		log_e( "ccamera_qhy8l::get_info(): get_dc201() failed" );
		return ret;
	}
...
int ccamera_qhy8l::get_dc201( short *dc )
{
...
double ccamera_qhy8l::r_to_degree( double R ) const
{
	double  T;
	double LNR;
...
double ccamera_qhy8l::mv_to_degree( double v ) const
{
	double R;
	double T;
...
int ccamera_qhy8l::set_dc201( int pwm )
{
