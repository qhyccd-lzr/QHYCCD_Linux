...
int main(int argc,char *argv[])
{
    ...
    if(found == 1)
    {
        ...
        ret = ControlQHYCCDShutter(camhandle,MACHANICALSHUTTER_CLOSE);
        if(ret == QHYCCD_SUCCESS)
        {
            printf("ControlQHYCCDShutter success!\n");
        }
        else
        {
            printf("It seems your camera does not have machanical shutter\n");
            printf("ControlQHYCCDShutter fail\n");
            goto failure;
        }
