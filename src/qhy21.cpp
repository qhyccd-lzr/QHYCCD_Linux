...
int QHY21::ConnectCamera(libusb_device *d,qhyccd_handle **h)
{
    int ret;
...
int QHY21::DisConnectCamera(qhyccd_handle *h)
{
    closeCamera(h);
    return QHYCCD_SUCCESS;
}

int QHY21::ReSetParams2cam(qhyccd_handle *h)
{
...
int QHY21::InitChipRegs(qhyccd_handle *h)
{
...
int QHY21::IsChipHasFunction(CONTROL_ID controlId)
{
...
int QHY21::GetControlMinMaxStepValue(CONTROL_ID controlId,double *min,double *max,double *step)
{
    int ret = QHYCCD_ERROR_NOTSUPPORT;
...
int QHY21::GetChipMemoryLength()
{
    return 2048 * 1500 * 3;
}

double QHY21::GetChipExposeTime()
{
    return camtime;
}

double QHY21::GetChipGain()
{
    return camgain;
}

double QHY21::GetChipOffset()
{
    return camoffset;
}

double QHY21::GetChipSpeed()
{
    return usbspeed;
}

double QHY21::GetChipBitsMode()
{
    return cambits;
}

double QHY21::GetChipCoolTemp(qhyccd_handle *h)
{
...
double QHY21::GetChipCoolPWM()
{
    return currentPWM;
}

int QHY21::SetChipGain(qhyccd_handle *h,double gain)
{
    int ret = QHYCCD_ERROR;
    camgain = gain;
...
int QHY21::SetChipOffset(qhyccd_handle *h,double offset)
{
    int ret = QHYCCD_ERROR;
    camoffset = offset;
...
int QHY21::SetChipExposeTime(qhyccd_handle *h,double time)
{
    int ret = QHYCCD_ERROR;
    camtime = time / 1000;
...
int QHY21::CorrectWH(int *w,int *h)
{
    return QHYCCD_SUCCESS;
}

int QHY21::InitBIN11Mode()
{
    ccdreg.HBIN = 1;
    ccdreg.VBIN = 1;
...
int QHY21::SetChipResolution(qhyccd_handle *h,int x,int y)
{
    int ret = QHYCCD_ERROR_NOTSUPPORT;
...
int QHY21::BeginSingleExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
    flagquit = false;
...
int QHY21::StopSingleExposure(qhyccd_handle *h)
{
    flagquit = true;
    sendForceStop(h);
    return QHYCCD_SUCCESS;  
...
int QHY21::GetSingleFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
...
int QHY21::BeginLiveExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
...
int QHY21::StopLiveExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
...
int QHY21::GetLiveFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
    return ret;  
}

int QHY21::SetChipSpeed(qhyccd_handle *h,int i)
{
    int ret = QHYCCD_ERROR;
...
int QHY21::SetChipBinMode(qhyccd_handle *h,int wbin,int hbin)
{
    int ret = QHYCCD_ERROR;
...
int QHY21::Send2CFWPort(qhyccd_handle *h,int pos)
{
    int ret;
    unsigned char buf[2];
...
int QHY21::AutoTempControl(qhyccd_handle *h,double ttemp)
{
    /* target temprature */
    targetTEMP = ttemp;
...
int QHY21::SetChipCoolPWM(qhyccd_handle *h,double PWM)
{
    int ret = QHYCCD_ERROR;
...
void QHY21::ConvertDataBIN11(unsigned char * Data,int x, int y, unsigned short PixShift)
{
...
void QHY21::ConvertDataBIN22(unsigned char * Data,int x, int y, unsigned short PixShift)
{
    unsigned char *Buf = NULL;
...
void QHY21::ConvertDataBIN44(unsigned char * Data,int x, int y, unsigned short PixShift)
{
     unsigned char * Buf = NULL;
     unsigned int pix;
