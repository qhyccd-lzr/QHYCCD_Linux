...
int QHYXXX::ConnectCamera(libusb_device *d,qhyccd_handle **h)
{
    int ret;
...
int QHYXXX::DisConnectCamera(qhyccd_handle *h)
{
    if(monoimg)
    {
...
int QHYXXX::ReSetParams2cam(qhyccd_handle *h)
{
...
int QHYXXX::InitChipRegs(qhyccd_handle *h)
{
...
int QHYXXX::IsChipHasFunction(CONTROL_ID controlId)
{
...
int QHYXXX::IsColorCam()
{
    return QHYCCD_MONO;
}

int QHYXXX::IsCoolCam()
{
    return QHYCCD_NOTCOOL;
}

int QHYXXX::GetControlMinMaxStepValue(CONTROL_ID controlId,double *min,double *max,double *step)
{
    int ret = QHYCCD_ERROR_NOTSUPPORT;
...
int QHYXXX::GetChipMemoryLength()
{
    return 1280 * 960 * 2;
}

double QHYXXX::GetChipExposeTime()
{
    return camtime;
}

double QHYXXX::GetChipGain()
{
    return camgain;
}

double QHYXXX::GetChipSpeed()
{
    return usbspeed;
}

double QHYXXX::GetChipUSBTraffic()
{
    return usbtraffic;
}

double QHYXXX::GetChipBitsMode()
{
    return cambits;
}

int QHYXXX::SetChipGain(qhyccd_handle *h,double gain)
{
    int ret = QHYCCD_ERROR;
...
int QHYXXX::SetChipExposeTime(qhyccd_handle *h,double time)
{
    // 需要输入的参数: cmosclk
    double cmosclk;
    double pixelPeriod;
...
int QHYXXX::CorrectWH(int *w,int *h)
{
    if(*w <= 320 && *h <= 240)
    {
	*w = 320;
...
int QHYXXX::Init1280x960(qhyccd_handle *h)
{
    int ret;
    int xstart;
    int ystart;
...
int QHYXXX::Init1024x768(qhyccd_handle *h)
{ 
    int xstart;
    int ystart;
    int ret;
...
int QHYXXX::Init800x600(qhyccd_handle *h)
{
    int xstart;
    int ystart;
    int ret;
...
int QHYXXX::Init640x480(qhyccd_handle *h)
{
    int xstart;
    int ystart;
    int ret;
...
int QHYXXX::Init320x240(qhyccd_handle *h)
{
    int xstart;
    int ystart;
    int ret;
...
int QHYXXX::SetChipResolution(qhyccd_handle *h,int x,int y)
{
    int ret;
    int xstart;
    int ystart;
...
int QHYXXX::SetChipUSBTraffic(qhyccd_handle *h,int i)
{
    int ret;
...
int QHYXXX::BeginSingleExposure(qhyccd_handle *h)
{
    flagquit = false;
    beginVideo(h);
    return QHYCCD_SUCCESS;
...
int QHYXXX::StopSingleExposure(qhyccd_handle *h)
{
    flagquit = true;
    sendForceStop(h);
    return QHYCCD_SUCCESS;  
...
int QHYXXX::GetSingleFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
    int exptime;
...
int QHYXXX::BeginLiveExposure(qhyccd_handle *h)
{
    flagquit = false;
    beginVideo(h);
    return QHYCCD_SUCCESS;
...
int QHYXXX::StopLiveExposure(qhyccd_handle *h)
{
    flagquit = true;
    sendForceStop(h);
    return QHYCCD_SUCCESS;  
...
int QHYXXX::GetLiveFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
    int exptime;
    int retevt;
...
int QHYXXX::SetChipSpeed(qhyccd_handle *h,int i)
{
    int ret;
    unsigned char buf[2];
    buf[0] = i;
...
int QHYXXX::SetChipBitsMode(qhyccd_handle *h,int bits)
{
    unsigned char buf[2];
...
int QHYXXX::SetChipBinMode(qhyccd_handle *h,int wbin,int hbin)
{
    return QHYCCD_SUCCESS;
}

int QHYXXX::Send2GuiderPort(qhyccd_handle *h,unsigned char Direction,unsigned short PulseTime)
{
...
int QHYXXX::SetPll(qhyccd_handle *h,unsigned char clk)
{
    double i = 0;
    if (clk == 0) 
    {
...
void QHYXXX::InitCmos(qhyccd_handle *h)
{
...
void QHYXXX::SWIFT_MSBLSB(unsigned char *ImgData)
{
    int i = 0;
    unsigned char temp;
