...
int QHY8PRO::ConnectCamera(libusb_device *d,qhyccd_handle **h)
{
    int ret;
...
int QHY8PRO::DisConnectCamera(qhyccd_handle *h)
{
    closeCamera(h);
    return QHYCCD_SUCCESS;
}

int QHY8PRO::ReSetParams2cam(qhyccd_handle *h)
{
...
int QHY8PRO::InitChipRegs(qhyccd_handle *h)
{
...
int QHY8PRO::IsChipHasFunction(CONTROL_ID controlId)
{
...
int QHY8PRO::IsColorCam()
{
    return QHYCCD_MONO;
}

int QHY8PRO::IsCoolCam()
{
    return QHYCCD_COOL;
}

int QHY8PRO::GetControlMinMaxStepValue(CONTROL_ID controlId,double *min,double *max,double *step)
{
    int ret = QHYCCD_ERROR_NOTSUPPORT;
...
int QHY8PRO::GetChipMemoryLength()
{
    return 3328 * 2030 * 3;
}

double QHY8PRO::GetChipExposeTime()
{
    return camtime;
}

double QHY8PRO::GetChipGain()
{
    return camgain;
}

double QHY8PRO::GetChipOffset()
{
    return camoffset;
}

double QHY8PRO::GetChipSpeed()
{
    return usbspeed;
}

double QHY8PRO::GetChipBitsMode()
{
    return cambits;
}

double QHY8PRO::GetChipCoolTemp(qhyccd_handle *h)
{
...
double QHY8PRO::GetChipCoolPWM()
{
    return currentPWM;
}

int QHY8PRO::SetChipGain(qhyccd_handle *h,double gain)
{
    int ret = QHYCCD_ERROR;
    camgain = gain;
...
int QHY8PRO::SetChipOffset(qhyccd_handle *h,double offset)
{
    int ret = QHYCCD_ERROR;
    camoffset = offset;
...
int QHY8PRO::SetChipExposeTime(qhyccd_handle *h,double time)
{
    int ret = QHYCCD_ERROR;
    camtime = time / 1000;
...
int QHY8PRO::CorrectWH(int *w,int *h)
{
    return QHYCCD_SUCCESS;
}

int QHY8PRO::InitBIN11Mode()
{
    ccdreg.HBIN = 1;
    ccdreg.VBIN = 1;
    ccdreg.LineSize = 6656;
    ccdreg.VerticalSize = 1015;
    ccdreg.TopSkipPix = 2300;
    psize= 26624;
    camxbin = 1;
    camybin = 1;
    camx = 3328;
...
int QHY8PRO::SetChipResolution(qhyccd_handle *h,int x,int y)
{
    int ret = QHYCCD_ERROR_NOTSUPPORT;
...
int QHY8PRO::BeginSingleExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
    flagquit = false;
...
int QHY8PRO::StopSingleExposure(qhyccd_handle *h)
{
    flagquit = true;
    sendForceStop(h);
    return QHYCCD_SUCCESS;  
...
int QHY8PRO::GetSingleFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
...
int QHY8PRO::BeginLiveExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
...
int QHY8PRO::StopLiveExposure(qhyccd_handle *h)
{
    int ret = QHYCCD_ERROR;
...
int QHY8PRO::GetLiveFrame(qhyccd_handle *h,int *pW,int *pH,int * pBpp,int *pChannels,unsigned char *ImgData)
{
    int ret = QHYCCD_ERROR;
    return ret;  
}

int QHY8PRO::SetChipSpeed(qhyccd_handle *h,int i)
{
    int ret = QHYCCD_ERROR;
...
int QHY8PRO::SetChipBinMode(qhyccd_handle *h,int wbin,int hbin)
{
    int ret = QHYCCD_ERROR;
...
int QHY8PRO::Send2CFWPort(qhyccd_handle *h,int pos)
{
    int ret;
    unsigned char buf[2];
...
int QHY8PRO::AutoTempControl(qhyccd_handle *h,double ttemp)
{
    /* target temprature */
    targetTEMP = ttemp;
...
int QHY8PRO::SetChipCoolPWM(qhyccd_handle *h,double PWM)
{
    int ret = QHYCCD_ERROR;
...
void QHY8PRO::ConvertDataBIN11(unsigned char * Data,int x, int y, unsigned short PixShift)
{
...
void QHY8PRO::ConvertDataBIN22(unsigned char * Data,int x, int y, unsigned short PixShift)
{
    unsigned char *Buf = NULL;
...
void QHY8PRO::ConvertDataBIN44(unsigned char * Data,int x, int y, unsigned short PixShift)
{
     unsigned char * Buf = NULL;
     unsigned int pix;
